...
- `Subtract 0.1 * the gradient of w from w.` gives `w` a new leaf with the new value, keeping its name and gradient. It is not an in-place edit: in-place would also change any variable sharing the node (`let b be … with value w.`), which plain decimals never do, and would leave already-built graphs holding a value they weren't computed from. On a derived value the same sentence builds a new node.
